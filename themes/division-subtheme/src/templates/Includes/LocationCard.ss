    ...
    <div class="card__body">
        ...
        <% if not $Parent.HideSummaries %>
            ...
            <% else %>
                ...
                <a href="$Link" class="button small hollow" aria-label="Continue reading about $Title">Continue Reading <i class="fas fa-arrow-right"></i></a>
            <% end_if %>
        <% end_if %>
    <% if $Categories.exists %>
        <p class="">
        <% loop $Categories %>
            <a href="$Link" class="button small hollow black">$Title</a><% if not Last %><% else %><% end_if %>
        <% end_loop %>
        </p>
    <% end_if %>
    </div>
