<header id="header" class="header header--dark-header header--gg-home" role="banner">
			<div class="nav__toggle nav__toggle--menu show-for-small hide-for-medium">
				<div class="nav__link nav__link--{$DarkLightHeader} nav__link--mobile-toggle" data-toggle="offCanvas"><span class="nav__menu-icon nav__menu-icon--{$DarkLightHeader}" id="nav__menu-icon"></span><span class="nav__menu-text nav__menu-text--{$DarkLightHeader}" id="nav__menu-text">Menu</span></div>
			</div>

			<div class="nav__wrapper nav__wrapper--overlay nav__wrapper--{$DarkLightHeader} nav__wrapper--gg-home" id="nav__wrapper">

				<nav role="nav" class="" aria-label="Main menu">

					<ul class="nav nav--{$DarkLightHeader} nav--no-bg clearfix" id="nav">
						<% loop $Menu(1) %>
						<li class="nav__item nav__item--{$Top.DarkLightHeader} <% if $FirstLast %>nav__item--$FirstLast<% end_if %><% if $Children %> nav__item--parent<% end_if %> nav__item--{$LinkOrCurrent} nav__item--{$LinkOrSection}">
							<a class="nav__link nav__link--{$Top.DarkLightHeader}<% if $Children %> nav__link--parent<% end_if %> nav__link--small" href="$Link">$MenuTitle</a>
							<% if $Children %>
								<% if $Children.Count > 4 %>
									<ul class="subnav subnav--{$Top.DarkLightHeader} subnav--two-columns">
										<% loop $Children %>
								...
								<% else %>
									<ul class="subnav subnav--{$Top.DarkLightHeader}">
										<% loop $Children %>
						...
						<li class="nav__item nav__item--{$DarkLightHeader} nav__search-item" id="nav__search-item" >
							<div class="nav__link nav__link--{$DarkLightHeader} nav__link--search">
			...
			<div class="nav__toggle nav__toggle--search show-for-small hide-for-medium">
				<div class="nav__link nav__link--{$DarkLightHeader}"><i class="fa fa-lg fa-search site-search-button" aria-hidden="true"></i>
				</div>
			</div>
